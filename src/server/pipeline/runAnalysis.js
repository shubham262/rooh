...
import { createExperience, findDuplicate } from "@/server/store/experienceStore";
...
export async function runAnalysis(rawUrl) {
	...
	// 5. Deduplicate on normalised URL first, then content fingerprint.
	const normalizedUrl = normalizeUrl(content.finalUrl || url);
	const fingerprint = buildFingerprint(extraction);
	const duplicate = findDuplicate({ normalizedUrl, fingerprint });
